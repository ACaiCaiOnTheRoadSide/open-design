...
export function openPgAsync(cfg: DaemonDbConfig, password: string): AsyncDb {
  ...
  // OD_PG_SCHEMA: pin every pooled connection to a dedicated schema so the
  // daemon's tables share the platform's single Postgres database without
  // colliding with the Go backend's public-schema tables (projects/
  // conversations/...). Ported from the retired sync worker (pg-sync-worker.ts).
  // The handler runs on each NEW connection; node-pg enqueues this query before
  // the acquirer's first query on the same client, so search_path is in effect
  // before any table access. CREATE SCHEMA IF NOT EXISTS is idempotent (runs at
  // most `max` times, once per pooled connection).
  if (pg.schema) {
    const ident = '"' + pg.schema.replace(/"/g, '""') + '"';
    pool.on('connect', (client) => {
      client
        .query(`CREATE SCHEMA IF NOT EXISTS ${ident}; SET search_path TO ${ident}`)
        .catch((err) => console.error('[pg-async] schema init failed:', err?.message ?? err));
    });
  }
