...
export function registerPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.get('/api/plugins', async (_req, res) => { try { res.json({ plugins: helpers.applyBakedPreviews(await plugins.listInstalledPlugins(db), helpers.PLUGIN_PREVIEWS_DIR) }); } catch (err) { res.status(500).json({ error: String(err) }); } });
  app.get('/api/plugins/:id', async (req, res) => { try { const plugin = await plugins.getInstalledPlugin(db, req.params.id); if (!plugin) return res.status(404).json({ error: 'plugin not found' }); res.json(plugin); } catch (err) { res.status(500).json({ error: String(err) }); } });
  ...
  app.post('/api/plugins/:id/duplicate-project', helpers.requireLocalDaemonRequest, async (req, res) => {
    try {
      ...
      const plugin = await plugins.getInstalledPlugin(db, pluginId);
      ...
      const project = await projectStore.insertProject(db, {
        id: projectId,
        name: projectName,
        skillId: null,
        designSystemId: null,
      ...
      await conversations.insertConversation(db, {
        id: conversationId,
        projectId,
        title: null,
        createdAt: now,
      ...
      const loadedProject = (await projectStore.getProject(db, projectId)) ?? project;
  ...
  app.get('/api/applied-plugins/:snapshotId', async (req, res) => { try { const snap = await plugins.getSnapshot(db, req.params.snapshotId); if (!snap) return res.status(404).json({ error: 'snapshot not found' }); res.json(snap); } catch (err) { res.status(500).json({ error: String(err) }); } });
  app.get('/api/applied-plugins/:snapshotId/canon', async (req, res) => { try { const snap = await plugins.getSnapshot(db, req.params.snapshotId); if (!snap) return res.status(404).json({ error: 'snapshot not found' }); const block = plugins.pluginPromptBlock(snap); const accepts = String(req.headers.accept ?? '').toLowerCase(); if (accepts.includes('text/plain')) { res.setHeader('Content-Type', 'text/plain; charset=utf-8'); res.send(block); return; } res.json({ snapshotId: snap.snapshotId, pluginId: snap.pluginId, block }); } catch (err) { res.status(500).json({ error: String(err) }); } });
  app.get('/api/applied-plugins', async (_req, res) => { try { const rows = (await db.prepare(`SELECT id FROM applied_plugin_snapshots ORDER BY applied_at DESC LIMIT 500`).all()) as SqliteRowId[]; const snapshots: AppliedPluginSnapshotLike[] = []; for (const r of rows) { const snap = await plugins.getSnapshot(db, r.id); if (snap !== null) snapshots.push(snap); } res.json({ snapshots }); } catch (err) { res.status(500).json({ error: String(err) }); } });
  app.get('/api/projects/:projectId/applied-plugins', async (req, res) => { try { const rows = (await db.prepare(`SELECT id FROM applied_plugin_snapshots WHERE project_id = ? ORDER BY applied_at DESC`).all(req.params.projectId)) as SqliteRowId[]; const snapshots: AppliedPluginSnapshotLike[] = []; for (const r of rows) { const snap = await plugins.getSnapshot(db, r.id); if (snap !== null) snapshots.push(snap); } res.json({ snapshots }); } catch (err) { res.status(500).json({ error: String(err) }); } });
...
export function registerProjectPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.get('/api/projects/:id/plugin-candidates', async (req, res) => { try { const project = await helpers.getProject(db, req.params.id); if (!project) return helpers.sendApiError(res, 404, 'PROJECT_NOT_FOUND', 'project not found'); const includeDismissed = req.query.includeDismissed === 'true'; res.json({ candidates: await plugins.listSkillPluginCandidates(db, req.params.id, includeDismissed) }); } catch (err: unknown) { res.status(400).json({ error: err instanceof Error ? err.message : String(err) }); } });
  app.post('/api/projects/:id/plugin-candidates/:candidateId/dismiss', async (req, res) => { if (!helpers.isLocalSameOrigin(req, helpers.resolvedPortRef.current)) return res.status(403).json({ error: 'cross-origin request rejected' }); const candidate = await plugins.dismissSkillPluginCandidate(db, req.params.id, req.params.candidateId); if (!candidate) return helpers.sendApiError(res, 404, 'NOT_FOUND', 'plugin candidate not found'); if (candidate.assistantMessageId) await db.prepare(`DELETE FROM messages WHERE id = ?`).run(candidate.assistantMessageId); res.json({ ok: true, candidate }); });
