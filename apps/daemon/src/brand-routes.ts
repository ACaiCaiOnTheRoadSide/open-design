...
async function createBrandStatusContext(deps: BrandRoutesDeps): Promise<BrandStatusContext> {
  ...
  for (const [conversationId, status] of (await listLatestConversationRunStatuses(deps.db)) as Map<string, BrandRunStatus>) {
    latestByConversation.set(conversationId, status);
  }
  const firstByConversation = new Map<string, BrandRunStatus>();
  for (const [conversationId, status] of (await listFirstConversationRunStatuses(deps.db)) as Map<string, BrandRunStatus>) {
    firstByConversation.set(conversationId, status);
  }
  const latestByRun = new Map<string, BrandRunStatus>();
  for (const [runId, status] of (await listLatestRunStatuses(deps.db)) as Map<string, BrandRunStatus>) {
  ...
  return {
    ...
    firstByConversation,
    latestByRun,
    awaitingInput: await listProjectsAwaitingInput(deps.db),
    awaitingInputByConversation: await listConversationsAwaitingInput(deps.db),
  };
}
