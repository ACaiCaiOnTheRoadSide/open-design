...
export async function startServer({
  ...
  try {
    seedLibraryExtensionOrigins(await listLibraryTokenOrigins(db));
  } catch {
  ...
  const pluginRouteHelpers = {
    ...
    installOrUpgradePlugin: async (req, res, mode) => {
      ...
      try {
        const basePlugin = mode === 'upgrade' ? await getInstalledPlugin(db, id) : null;
    ...
    handleShareProject: async (req, res) => {
      try {
        ...
        await insertProject(db, { id, name: `${PLUGIN_SHARE_ACTION_LABELS[action]}: ${sourcePlugin.title || sourcePlugin.id}`, skillId: null, designSystemId: null, pendingPrompt: prompt, metadata, createdAt: now, updatedAt: now });
        await insertConversation(db, { id: cid, projectId: id, title: null, createdAt: now, updatedAt: now });
        const registry = await loadPluginRegistryView(); const connectorProbe = buildConnectorProbe(connectorService); const resolved = await resolvePluginSnapshot({ db, body: { pluginId: actionPluginId, pluginInputs: { source_plugin_id: sourcePlugin.id, source_plugin_title: sourcePlugin.title || sourcePlugin.id, source_plugin_version: sourcePlugin.version, source_plugin_path: sourcePlugin.fsPath, plugin_context_path: stagedPath }, locale: typeof body.locale === 'string' ? body.locale : undefined }, projectId: id, conversationId: cid, registry, connectorProbe });
        if (resolved && !resolved.ok) return res.status(resolved.status).json(resolved.body);
        const project = await getProject(db, id); if (!project) return sendApiError(res, 500, 'INTERNAL_ERROR', 'created project could not be loaded');
    ...
    handlePluginTrust: async (req, res) => {
      try {
        const plugin = await getInstalledPlugin(db, req.params.id); if (!plugin) return res.status(404).json({ error: 'plugin not found' });
        ...
        const next = action === 'revoke' ? await revokeCapabilities({ db, pluginId: req.params.id, capabilities: accepted }) : await grantCapabilities({ db, pluginId: req.params.id, capabilities: accepted });
    ...
    handlePluginStats: async (res) => {
      try { const { pluginInventoryStats, snapshotInventoryStats } = await import('./plugins/stats.js'); const installed = await listInstalledPlugins(db); const inventoryRows = await db.prepare(`SELECT status, project_id, run_id, applied_at FROM applied_plugin_snapshots`).all(); res.json({ plugins: pluginInventoryStats(installed), snapshots: snapshotInventoryStats(inventoryRows), generatedAt: Date.now() }); } catch (err) { res.status(500).json({ error: String(err) }); }
    },
    ...
    if (
      ...
      try {
        pluginDesignSystemId = designSystemIdFromPluginSnapshot(
          await getSnapshot(db, appliedPluginSnapshotId),
        );
      } catch (err) {
        console.warn(
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // 计量归因(baizhi fork):spawn 实际使用的模型挂到 run 上,
    ...
    const spawnRetryAttempt = () => {
      // catch 必须 async:体内 await finishWithRetryDecision 持久化失败终态
    ...
    try {
      ...
      // Admission control. Everything above this line is cheap preparation;
      // everything below holds a child process, a buffered prompt, and a live
      ...
      if (!gateSlot || run.cancelRequested) {
        ...
        revokeToolToken('child_exit');
        unregisterChatAgentEventSink();
        return design.runs.finish(run, 'canceled', null, null);
      }
      // finish() is the sole release point and runs exactly once per run.
