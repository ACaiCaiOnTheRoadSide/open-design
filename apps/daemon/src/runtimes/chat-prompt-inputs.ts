...
export function formatLazyHydrationHint(
  ...
  return [
    ...
    ...largeFiles.map((file) => `- \`${file.path}\` (${formatProjectEntrySize(file.size)})`),
    '',
...
export function formatDesignFilesWorkspaceHint(
  ...
  const lines = [
    ...
    `The Design Files panel is backed by your current working directory: \`${cwd}\`. Every deliverable and generated or downloaded resource must be saved inside this directory so it can be synchronized back to OpenDesign. The user can browse these files in real time.`,
    `When a file-writing tool accepts a path, use an absolute path rooted at the current working directory (for example \`${cwd}/investor-pitch-deck.html\` or \`${cwd}/assets/x.png\`). Never write project output to a filesystem-root path such as \`/assets/x.png\`, another temporary directory, or a hidden \`.assets/\` directory.`,
    'Inside HTML, CSS, and other project files, reference those resources with project-relative URLs such as `assets/x.png`, not filesystem absolute paths. After generating or downloading a resource, verify that it exists under the current working directory before reporting success.',
