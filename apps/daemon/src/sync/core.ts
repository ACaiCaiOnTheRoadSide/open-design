...
/** Normalize untrusted JSON/state maps without inheriting Object.prototype. */
export function normalizeManifestFiles(input: unknown): ManifestFiles {
  const files = Object.create(null) as ManifestFiles;
  if (!input || typeof input !== 'object' || Array.isArray(input)) return files;
  for (const [relPath, raw] of Object.entries(input)) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) continue;
    const value = raw as Record<string, unknown>;
    if (typeof value.sha256 !== 'string' || typeof value.size !== 'number' || typeof value.mtime !== 'number') continue;
    files[relPath] = { sha256: value.sha256, size: value.size, mtime: value.mtime };
  }
  return files;
}
...
/** Local-only trees are never valid manifest entries, on any filesystem. */
export function isSyncIgnoredPath(relPath: string): boolean {
  const firstSegment = relPath.split('/')[0]?.toLowerCase();
  return typeof firstSegment === 'string' && SYNC_IGNORED_DIR_NAMES.has(firstSegment);
}
  ...
  const walk = async (dir: string, relPrefix: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (isSyncIgnoredPath(rel)) continue;
...
export function diffManifests(base: ManifestFiles, local: ManifestFiles): ManifestOp[] {
  ...
  for (const [relPath, entry] of Object.entries(local)) {
    const before = Object.hasOwn(base, relPath) ? base[relPath] : undefined;
  ...
  for (const relPath of Object.keys(base)) {
    if (!Object.hasOwn(local, relPath)) ops.push({ op: 'delete', path: relPath });
  }
  return ops;
}
 ...
 */
export function manifestPathShapeConflict(
  relPath: string,
  files: ManifestFiles,
  ignoredPaths: ReadonlySet<string> = new Set(),
): string | null {
  for (const existingPath of Object.keys(files)) {
    if (existingPath === relPath || ignoredPaths.has(existingPath)) continue;
    if (existingPath.startsWith(`${relPath}/`) || relPath.startsWith(`${existingPath}/`)) {
      return existingPath;
    }
  }
  return null;
...
export function rebaseOps(
  ...
  const deletedPaths = new Set(
    ops.filter((op): op is Extract<ManifestOp, { op: 'delete' }> => op.op === 'delete').map((op) => op.path),
  );
  for (const op of ops) {
    const current = Object.hasOwn(currentFiles, op.path) ? currentFiles[op.path] : undefined;
    if (op.op === 'put') {
      if (current && current.sha256 === op.sha256) continue; // already there
      const conflict = manifestPathShapeConflict(op.path, currentFiles, deletedPaths);
      if (conflict) {
        throw new Error(`sync conflict: paths ${JSON.stringify(op.path)} and ${JSON.stringify(conflict)} cannot both be materialized`);
      }
      rebased.push(op);
    } else {
      if (!current) continue; // already gone
      const basedOn = Object.hasOwn(baseFiles, op.path) ? baseFiles[op.path] : undefined;
...
export function applyOps(files: ManifestFiles, ops: ManifestOp[]): ManifestFiles {
  const next = normalizeManifestFiles(files);
  for (const op of ops) {
    if (op.op === 'put') {
...
export async function mapWithConcurrency<T, R>(
  ...
  const workers = Array.from({ length: Math.max(1, Math.min(limit, items.length)) }, async () => {
    while (!hasError && next < items.length) {
      const index = next;
      next += 1;
      try {
