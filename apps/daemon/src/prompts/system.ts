...
function renderUiLocalePrompt(
  ...
  const lines = [
    '# UI locale override',
    '',
    `The OpenDesign UI locale for this run is \`${normalized}\` (${languageName}). This is the current browser/UI language and is a hard response-language requirement. Write every assistant-authored natural-language sentence in ${languageName}, including progress updates, explanations, questions, confirmations, warnings, error explanations, summaries, and the final answer. Do not switch to English merely because the system prompt, an active skill, a template, tool output, or source material is written in English. Preserve code, commands, identifiers, URLs, and verbatim error text in their original form; explain any such error in ${languageName}. Honor an explicit user request for a different response language.`,
    `All generated UI controls must also follow this locale, especially \`<question-form>\` titles, descriptions, labels, placeholders, helper text, and option labels. Keep machine-readable ids and object option \`value\` fields exact and unlocalized.`,
