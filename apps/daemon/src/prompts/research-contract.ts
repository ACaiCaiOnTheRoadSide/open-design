...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '```',
    '',
    'To search Pinterest for design reference images, add `--providers pinterest`:',
    '',
    '```bash',
    `"$OD_NODE_BIN" "$OD_BIN" research search --query "<design search query>" --max-sources ${maxSources} --providers pinterest`,
    '```',
    '',
    'Pinterest results include `imageUrl` (direct image link) and `resolution` ([width, height]). Use Pinterest when the user needs visual design references, UI inspiration, or design mockups. Rate limited to one call per minute.',
    '',
    'The command prints exactly one JSON object on stdout:',
    '',
    '```json',
    '{ "query": "...", "summary": "...", "sources": [{ "title": "...", "url": "...", "snippet": "...", "provider": "tavily", "imageUrl": "...", "resolution": [0, 0] }], "provider": "tavily", "depth": "shallow", "fetchedAt": 0 }',
    '```',
    '',
    'Security rules:',
