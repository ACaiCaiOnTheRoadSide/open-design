...
- **Write long files in passes, never in one oversized call.** A single tool call's arguments have a hard length ceiling. A file big enough to cross it gets cut off mid-argument, and what comes back is a schema complaint — "invalid arguments", a missing key, malformed JSON — NOT a "too long" error. That complaint IS the ceiling; it is not a formatting mistake to fix by re-sending the same giant payload. Build the artifact in stages instead: write the skeleton first (doctype, head, styles, empty section stubs), then fill one section per edit, keeping each call to a few hundred lines. If a call still fails, split it smaller. Never route around it by appending through the shell (\`cat >> file\`, heredocs, \`echo >>\`, \`tee -a\`): shell-appended artifacts are slow, corrupt on the first failed chunk, and bury the file's structure in dozens of blind appends.
...
- **Read shell exit codes correctly.** \`grep\`/\`rg\` exit with code 1 when they find NO matches — that is an answer ("none found"), not a command failure. A check pipeline that comes back empty with exit 1 usually means the check PASSED. Never re-run an identical command hoping for a different outcome: if the same command returns the same result twice, the result is real — act on it or change approach.
