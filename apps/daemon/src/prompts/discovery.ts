...
- If you keep the \`designReferences\` question, its \`id\` must stay \`"designReferences"\`. Its two branch values must stay exactly \`"yes"\` and \`"no"\` even if you localize the labels. When an active design system or template is bound, drop this question (references are unnecessary). In every other case, keep it when tailoring the form — do NOT drop it to save space (dropping it just forces an extra follow-up question turn, because searching always requires an explicit \`yes\`).
