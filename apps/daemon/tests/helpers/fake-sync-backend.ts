...
import type { ManifestFiles, ManifestOp } from '../../src/sync/core.js';
    ...
    req.on('end', () => {
      ...
      if (url.pathname === '/api/internal/sync/manifest/diff' && req.method === 'POST') {
        ...
        for (const op of payload.ops) {
          if (op.op === 'put') files[op.path] = { sha256: op.sha256, size: op.size, mtime: op.mtime };
          else delete files[op.path];
        }
