...
describe('GET /api/projects/:id/raw/* range request route', () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(
      path.join(dir, 'usd-powered.html'),
      Buffer.from('<!doctype html><script type="module">import { USDLoader } from "./USDLoader.js"; new USDLoader().load("./assets/model.usdc");</script>'),
    );
  ...
  it('routes USDLoader previews through powered mode when the marker is missing', async () => {
    const res = await fetch(`${baseUrl}/api/projects/${projectId}/text-preview/usd-powered.html?limit=1024`);
    expect(res.status).toBe(200);
    const body = await res.json() as {
      poweredPreview: { required: boolean; complete: boolean };
    };
