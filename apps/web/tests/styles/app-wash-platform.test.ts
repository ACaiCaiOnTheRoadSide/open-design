...
describe('desktop app wash platform contract', () => {
  it('uses the design-prompt lunar globe only for explicit or system dark mode', () => {
    ...
    expect(existsSync(new URL('../../public/backgrounds/lunar-globe-rotation-1280.mp4', import.meta.url))).toBe(true);
    expect(existsSync(new URL('../../public/backgrounds/lunar-globe-rotation-3k.mp4', import.meta.url))).toBe(true);
    expect(existsSync(new URL('../../public/backgrounds/lunar-globe-rotation-4k.mp4', import.meta.url))).toBe(true);
    ...
    expect(lunarSceneSource).toContain('src="/backgrounds/lunar-globe-rotation-1280.mp4"');
    expect(lunarSceneSource).toContain('src="/backgrounds/lunar-globe-rotation-3k.mp4"');
    expect(lunarSceneSource).toContain('src="/backgrounds/lunar-globe-rotation-4k.mp4"');
    expect(lunarSceneSource).toContain('(min-resolution: 2dppx)');
