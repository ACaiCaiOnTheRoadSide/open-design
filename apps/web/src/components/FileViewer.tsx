...
function HtmlViewer({
  ...
  const captureExportImageSnapshot = useCallback(async (
    ...
    // 白标/纯 web 修复:桌面合成器只看得到屏内像素,整页导出(fullPage)时跳过它,
    // 走 iframe 内 foreignObject 桥拿全文档高度。
    if (!options?.fullPage) {
    ...
    const bridgeOpts = { fullPage: options?.fullPage === true };
    if (!useUrlLoadPreview) {
      ...
      await waitForAnimationFrame();
      return requestPreviewSnapshotWithRetry(activeIframe, bridgeOpts);
    }
    ...
    if (urlIframe) {
      ...
      await waitForAnimationFrame();
      const urlSnapshot = await requestPreviewSnapshotWithRetry(urlIframe, bridgeOpts);
    ...
    try {
      await waitForAnimationFrame();
      return requestPreviewSnapshotWithRetry(srcDocIframe, bridgeOpts);
    } finally {
      restoreVisibility();
    }
  }, [
  ...
  async function handleImageExportSave() {
    ...
    try {
      ...
      if (!dataUrl) {
        ...
        const snap = await captureExportImageSnapshot({ wholeDeck: true, fullPage: true });
        if (!snap) {
