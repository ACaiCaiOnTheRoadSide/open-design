...
const SAFE_ATTACHMENT_TYPES = new Set(['image/png', 'image/jpeg', 'image/webp', 'image/gif']);
  ...
  const addFiles = useCallback(
    (files: File[]) => {
      ...
      for (const file of files) {
        ...
        if (!SAFE_ATTACHMENT_TYPES.has(file.type.toLowerCase()) || file.size > MAX_ATTACHMENT_BYTES) {
          rejected = true;
          continue;
        }
  ...
  const onPaste = useCallback(
    (event: ClipboardEvent<HTMLDivElement>) => {
      const files = Array.from(event.clipboardData?.files ?? []).filter((file) =>
        SAFE_ATTACHMENT_TYPES.has(file.type.toLowerCase()),
      );
      if (!files.length) return;
