...
export function isSafeDesignReferenceImage(image: string): boolean {
  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(image) || image.startsWith('//')) return false;
  const path = image.startsWith('/') ? image.slice(1) : image;
  if (!path || path.startsWith('?') || path.startsWith('#')) return false;
  try {
    return decodeURIComponent(path).split('/').every((segment) => segment !== '..' && segment !== '.');
  } catch {
    return false;
  }
}
...
function tryParseRefs(body: string): DesignReferences | null {
  ...
  for (const raw of rawItems) {
    ...
    if (!id || !title || !isSafeDesignReferenceImage(image) || seenIds.has(id)) continue;
    seenIds.add(id);
  ...
  const pageSize = typeof obj.pageSize === 'number' && Number.isFinite(obj.pageSize)
    ? Math.min(MAX_PAGE_SIZE, Math.max(1, Math.floor(obj.pageSize)))
    : undefined;
