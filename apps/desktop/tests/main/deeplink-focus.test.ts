...
describe("focusDesktopForDeeplink", () => {
  // A packaged cold start puts the splash ahead of the main window, so any
  // bring-to-front that selects a window itself can land on the wrong one and
  // silently do nothing. Routing through the runtime's `show()` is what makes
  // the `workspace/open` hand-off actually surface the app.
