...
// would exceed encoder limits (page/deck too tall for one image; PDF remains available).
...
if (!projectRoot || relativeOutput === '..' || relativeOutput.startsWith(`..${path.sep}`) || path.isAbsolute(relativeOutput)) {
  console.error(`unsafe output path outside project directory: ${output}`);
  process.exit(2);
}
  ...
  if (!deckInfo.isDeck) {
    ...
    fs.writeFileSync(
      ...
      '<!doctype html><html><head><meta charset="utf-8"></head>' +
        ...
        `<style>@page{size:${stage.w}px ${stage.h}px;margin:0}html,body{margin:0;padding:0}` +
        `img{display:block;width:${stage.w}px;height:${stage.h}px;break-after:page;page-break-after:always}` +
        'img:last-child{break-after:auto;page-break-after:auto}</style>' +
        shots
          .map(
            (shot) =>
              `<img src="${path.basename(shot)}" alt="">`,
          )
          .join('') +
        '</body></html>',
    );
