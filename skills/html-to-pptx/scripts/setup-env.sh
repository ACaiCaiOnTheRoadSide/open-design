# setup-env.sh — one-shot environment prep for the html-to-pptx skill.
#
...
if [ -z "$BROWSER" ] && [ -f /etc/alpine-release ]; then
  ...
  # A system chromium exists — pin it explicitly for render-pptx.mjs, whose
  # own fallback detection only probes /usr/bin paths, not the whole PATH.
  echo "export OD_PPTX_CHROMIUM=\"$BROWSER\"" >> "$WORKDIR/env.sh"
fi

echo "ok: pptx export environment ready — source $WORKDIR/env.sh before rendering"
